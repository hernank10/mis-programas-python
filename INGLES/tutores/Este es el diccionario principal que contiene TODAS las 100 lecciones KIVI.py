...
class GrammarApp(App):
    # Propiedades para actualizar el texto en la interfaz de usuario
    ...
    def build(self):
        ...
        # Contenedor para la teoría y el ejemplo
        self.info_layout = BoxLayout(orientation='vertical', size_hint=(1, 0.7), spacing=5)

        # Título de la lección
        self.title_label = Label(text=self.title_text, font_size='24sp', bold=True)
        self.info_layout.add_widget(self.title_label)

        # Teoría de la lección
        self.theory_label = Label(text=self.theory_text, font_size='16sp', text_size=(self.root_window.width, None),
                                  halign='left', valign='top')
        self.info_layout.add_widget(self.theory_label)

        # Ejemplo de la lección
        self.example_label = Label(text=self.example_text, font_size='16sp', italic=True, text_size=(self.root_window.width, None),
                                   halign='left', valign='top')
        self.info_layout.add_widget(self.example_label)

        # Contenedor para la pregunta y las opciones
        self.question_layout = BoxLayout(orientation='vertical', size_hint=(1, 0.3), spacing=10)
        self.question_label = Label(text=self.question_text, font_size='18sp')
        self.question_layout.add_widget(self.question_label)

        self.option_buttons_layout = BoxLayout(orientation='horizontal', spacing=10)
        self.option_buttons = []
        for i in range(3):
            btn = Button(text="", on_press=self.check_answer, background_color=(0.2, 0.6, 0.2, 1))
            self.option_buttons.append(btn)
            self.option_buttons_layout.add_widget(btn)
        self.question_layout.add_widget(self.option_buttons_layout)

        # Mensaje de retroalimentación
        self.feedback_label = Label(text="", font_size='16sp', bold=True, size_hint_y=0.1)

        self.main_layout.add_widget(self.info_layout)
        self.main_layout.add_widget(self.question_layout)
    ...
    def load_lesson(self):
        """Carga el contenido de la lección actual."""
        if self.leccion_actual_index < len(self.lecciones):
            ...
            # Actualiza las propiedades y los widgets
            self.title_label.text = leccion_titulo
            self.theory_label.text = leccion_data["teoría"]
            self.example_label.text = leccion_data["ejemplo"]
            self.question_label.text = leccion_data["pregunta"]
    ...
    def check_answer(self, instance):
        """Comprueba si la opción seleccionada es correcta y muestra un pop-up."""
        leccion_titulo = self.lecciones[self.leccion_actual_index]
        leccion_data = GRAMMAR_LESSONS[leccion_titulo]
        
        # Deshabilita los botones después de la respuesta
        for btn in self.option_buttons:
            btn.disabled = True

        if instance.text == leccion_data["respuesta_correcta"]:
            self.puntaje += 1
            self.feedback_label.text = "¡Correcto! Sigue así."
            self.feedback_label.color = (0, 1, 0, 1)  # Verde
        else:
            self.feedback_label.text = f"Incorrecto. La respuesta correcta es '{leccion_data['respuesta_correcta']}'"
            self.feedback_label.color = (1, 0, 0, 1)  # Rojo
        
        # Muestra un pop-up con la retroalimentación
        content = BoxLayout(orientation='vertical', spacing=10, padding=10)
        content.add_widget(Label(text=self.feedback_label.text, color=self.feedback_label.color))
        next_button = Button(text="Siguiente Lección", size_hint=(1, 0.3), background_color=(0.1, 0.5, 0.8, 1))
        content.add_widget(next_button)

        popup = Popup(title='Resultado', content=content, size_hint=(0.8, 0.4), auto_dismiss=False)
        next_button.on_press = lambda *args: (popup.dismiss(), self.next_lesson())
        popup.open()
    ...
    def show_end_message(self):
        """Muestra un mensaje cuando se completan todas las lecciones."""
        self.main_layout.clear_widgets()

        end_label = Label(text="¡Has completado todas las lecciones!", font_size='24sp', bold=True)
        self.main_layout.add_widget(end_label)
        
        score_label = Label(text=f"Tu puntuación final es: {self.puntaje}/{len(self.lecciones)}", font_size='18sp')
        self.main_layout.add_widget(score_label)
        
        restart_button = Button(text="Reiniciar", on_press=self.restart_app, size_hint=(0.5, 0.2), background_color=(0.2, 0.6, 0.2, 1))
        self.main_layout.add_widget(restart_button)

    def restart_app(self, instance):
        """Reinicia la aplicación al principio."""
        self.leccion_actual_index = 0
        self.puntaje = 0
        self.main_layout.clear_widgets()
        self.build()
