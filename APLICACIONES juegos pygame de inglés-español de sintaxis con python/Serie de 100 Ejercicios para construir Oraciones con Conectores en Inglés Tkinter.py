...
# Diseñado para hablantes de español, con una interfaz gráfica Tkinter.
#
...
class EnglishConnectorsApp:
    def __init__(self, root):
        """
        Inicializa la aplicación Tkinter.
        """
        self.root = root
        self.root.title("Reto de Conectores en Inglés")
        ...
        self.current_exercise_number = None
        ...
        self.title_label = tk.Label(main_frame, text="Reto de Conectores en Inglés", font=self.title_font, bg="#e0e0e0")
        self.title_label.pack(pady=(0, 20))

        # Etiqueta para mostrar el tipo de conector
        self.type_label = tk.Label(main_frame, text="", font=self.base_font, bg="#e0e0e0")
        self.type_label.pack()

        # Etiqueta para la tarea o pregunta
        self.task_label = tk.Label(main_frame, text="", font=self.base_font, bg="#e0e0e0")
        self.task_label.pack(pady=(10, 10))

        # Campo de entrada para la respuesta del usuario
        self.answer_entry = tk.Entry(main_frame, width=50, font=self.base_font)
        self.answer_entry.pack(pady=(0, 10))
        self.answer_entry.bind("<Return>", lambda event: self.check_answer())

        # Botón para comprobar la respuesta
        self.check_button = tk.Button(main_frame, text="Comprobar", command=self.check_answer, font=self.base_font, bg="#4CAF50", fg="white", activebackground="#45a049", borderwidth=0, relief="flat")
        self.check_button.pack(pady=(0, 10))

        # Etiqueta para mostrar la retroalimentación
        self.feedback_label = tk.Label(main_frame, text="", font=self.base_font, bg="#e0e0e0")
        self.feedback_label.pack(pady=(0, 10))

        # Botón para el siguiente ejercicio
        self.next_button = tk.Button(main_frame, text="Siguiente Ejercicio", command=self.next_exercise, font=self.base_font, bg="#008CBA", fg="white", activebackground="#007a99", borderwidth=0, relief="flat")
    ...
    def next_exercise(self):
        ...
        self.type_label.config(text=f"[Ejercicio {self.current_exercise_number}: {title}]")
    ...
    def check_answer(self):
        ...
        """
        user_response = self.answer_entry.get().strip()
        ...
        if clean_user_response == clean_correct_answer or (clean_user_response + '.') == clean_correct_answer or (clean_user_response + '?') == clean_correct_answer:
            self.feedback_label.config(text="¡Correcto! ¡Excelente trabajo!", fg="green")
        else:
            self.feedback_label.config(text=f"Incorrecto. La respuesta esperada era: '{self.correct_answer}'", fg="red")

if __name__ == "__main__":
    root = tk.Tk()
    app = EnglishConnectorsApp(root)
    root.mainloop()
