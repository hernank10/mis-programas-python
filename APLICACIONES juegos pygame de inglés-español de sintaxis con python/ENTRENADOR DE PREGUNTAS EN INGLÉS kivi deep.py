...
# Paleta de colores de la naturaleza
NATURE_COLORS = {
    'forest_dark': '#2D5A27',      # Verde bosque oscuro
    'forest_medium': '#4A7C59',    # Verde bosque medio
    'forest_light': '#8FB996',     # Verde bosque claro
    'sky_dark': '#1E3A5F',         # Azul cielo oscuro
    'sky_medium': '#3A6EA5',       # Azul cielo medio
    'sky_light': '#87CEEB',        # Azul cielo claro
    'earth_dark': '#8B4513',       # Marrón tierra oscuro
    'earth_medium': '#A0522D',     # Marrón tierra medio
    'earth_light': '#D2B48C',      # Beige tierra claro
    'sunset_dark': '#E2725B',      # Naranja atardecer oscuro
    'sunset_medium': '#F4A261',    # Naranja atardecer medio
    'sunset_light': '#F9C74F',     # Amarillo atardecer claro
    'water_dark': '#1A535C',       # Azul agua oscuro
    'water_medium': '#4ECDC4',     # Turquesa agua medio
    'water_light': '#A8DADC',      # Azul agua claro
    'stone_dark': '#6D6875',       # Gris piedra oscuro
    'stone_medium': '#B5838D',     # Rosa piedra medio
    'stone_light': '#FFCDB2',      # Rosa piedra claro
    'leaf_dark': '#386641',        # Verde hoja oscuro
    'leaf_medium': '#6A994E',      # Verde hoja medio
    'leaf_light': '#A7C957',       # Verde hoja claro
    'background_light': '#F8F9FA', # Fondo claro (nieve)
    'background_medium': '#E9ECEF',# Fondo medio (nube)
    'text_dark': '#2B2D42',        # Texto oscuro
...
class ExerciseWidget(BoxLayout):
    def __init__(self, exercise, **kwargs):
        ...
        with self.canvas.before:
            Color(rgb=get_color_from_hex(NATURE_COLORS['background_light']))
        ...
        type_layout.add_widget(Label(
            ...
            markup=True,
            font_size=sp(18),
            color=get_color_from_hex(NATURE_COLORS['forest_dark']),
            size_hint_x=0.7
        ))
        
        # Color de dificultad basado en la naturaleza
        difficulty_color = {
            'básico': NATURE_COLORS['leaf_medium'],
            'intermedio': NATURE_COLORS['sunset_medium'],
            'avanzado': NATURE_COLORS['sky_medium']
        }.get(exercise.difficulty, NATURE_COLORS['text_medium'])
        ...
        self.add_widget(Label(
            ...
            font_size=sp(16),
            color=get_color_from_hex(NATURE_COLORS['text_dark']),
            size_hint_y=None,
            height=dp(40)
        ))
        ...
        self.add_widget(Label(
            ...
            markup=True,
            font_size=sp(16),
            color=get_color_from_hex(NATURE_COLORS['water_dark']),
            size_hint_y=None,
            height=dp(60)
        ))
...
class StatsModal(ModalView):
    def __init__(self, stats_data, **kwargs):
        ...
        # Fondo natural
        with self.canvas.before:
            Color(rgb=get_color_from_hex(NATURE_COLORS['background_medium']))
            self.rect = Rectangle(pos=self.pos, size=self.size)
        self.bind(pos=self.update_rect, size=self.update_rect)
        ...
        for stat in stats_data:
            ...
            stats_layout.add_widget(Label(
                text=str(stat["value"]),
                font_size=sp(16),
                color=get_color_from_hex(NATURE_COLORS['forest_medium']),
                size_hint_y=None,
                height=dp(30)
            ))
        ...
        close_btn = Button(
            ...
            size_hint_y=None,
            height=dp(50),
            background_color=get_color_from_hex(NATURE_COLORS['sky_medium'])
        )
...
class EnglishQuestionsApp(App):
    ...
    def build(self):
        # Configurar la ventana con colores naturales
        Window.clearcolor = get_color_from_hex(NATURE_COLORS['background_light'])
        self.title = "🌿 Entrenador de Preguntas en Inglés - Naturaleza"
    ...
    def setup_header(self):
        ...
        # Fondo del header con gradiente de bosque
        with header.canvas.before:
            Color(rgb=get_color_from_hex(NATURE_COLORS['forest_dark']))
            header.rect = Rectangle(pos=header.pos, size=header.size)
        header.bind(pos=header.update_rect, size=header.update_rect)
        ...
        title = Label(
            ...
            markup=True,
            font_size=sp(20),
            color=get_color_from_hex(NATURE_COLORS['text_light']),
            size_hint_y=None,
            height=dp(40)
        )
    ...
    def setup_answer_area(self):
        ...
        self.answer_input = TextInput(
            ...
            size_hint_y=0.7,
            font_size=sp(16),
            background_color=get_color_from_hex(NATURE_COLORS['background_medium']),
            foreground_color=get_color_from_hex(NATURE_COLORS['text_dark']),
            cursor_color=get_color_from_hex(NATURE_COLORS['forest_medium'])
        )
    ...
    def check_answer(self, instance):
        ...
        if not user_answer:
            self.feedback_label.text = f'[color={NATURE_COLORS["sunset_dark"]}]🌵 Por favor, escribe una respuesta.[/color]'
            return
        ...
        if is_correct:
            self.feedback_label.text = f'[color={NATURE_COLORS["leaf_medium"]}]🌱 ¡CORRECTO![/color]'
            ...
            self.feedback_label.text = f'[color={NATURE_COLORS["sunset_dark"]}]🍂 Incorrecto. Intenta nuevamente.[/color]'
    ...
    def show_correct_answer(self, exercise):
        self.feedback_label.text = f'[color={NATURE_COLORS["sky_medium"]}]💧 Respuesta correcta: {exercise.correct_answer}\n\n🌿 {exercise.hint}[/color]'
    ...
    def show_hint(self, instance):
        exercise = self.all_exercises[self.current_exercise]
        self.hints_used += 1
        self.feedback_label.text = f'[color={NATURE_COLORS["sunset_medium"]}]🌅 PISTA: {exercise.hint}[/color]'
    ...
    def show_statistics(self, instance):
        stats_data = [
            ...
            {"label": "⏱️ Tiempo transcurrido", "value": self.time_elapsed},
...
"""
        if accuracy >= 90:
            result_text += "🏔️ ¡EXCELENTE! Dominas las preguntas en inglés"
        elif accuracy >= 70:
        ...
        # Fondo natural para el modal
        with result_modal.canvas.before:
            Color(rgb=get_color_from_hex(NATURE_COLORS['background_medium']))
            result_modal.rect = Rectangle(pos=result_modal.pos, size=result_modal.size)
        result_modal.bind(pos=result_modal.update_rect, size=result_modal.update_rect)
