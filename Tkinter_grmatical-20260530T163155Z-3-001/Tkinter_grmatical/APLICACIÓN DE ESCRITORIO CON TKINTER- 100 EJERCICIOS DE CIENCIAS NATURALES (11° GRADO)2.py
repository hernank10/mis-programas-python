...
class CienciasApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.parafrasis_font = font.Font(family="Helvetica", size=11, slant="italic")

        # Variables de estado
        self.ejercicios_completados = 0
        self.parafrasis_guardadas = []
    ...
    def crear_widgets(self):
        ...
        parafrasis_frame = tk.LabelFrame(main_frame, text="Escribe tu paráfrasis aquí", font=self.content_font, bg="white", padx=10, pady=10)
        parafrasis_frame.pack(fill="both", expand=True, pady=(0, 20))

        self.parafrasis_text = scrolledtext.ScrolledText(parafrasis_frame, wrap=tk.WORD, width=60, height=8, font=self.parafrasis_font)
        self.parafrasis_text.pack(fill="both", expand=True)
        ...
        self.boton_guardar = tk.Button(main_frame, text="Guardar todas las paráfrasis", command=self.guardar_parafrasis, font=self.content_font, bg="#2196F3", fg="white", relief="raised", bd=3)
    ...
    def cargar_ejercicio(self):
        ...
        if self.ejercicios_completados < len(self.ejercicios):
            ...
            self.parafrasis_text.delete("1.0", tk.END)
        else:
            self.ejercicio_label.config(text="¡Has completado todos los ejercicios! 🎉")
            self.parafrasis_text.delete("1.0", tk.END)
            self.parafrasis_text.config(state=tk.DISABLED)
    ...
    def siguiente_ejercicio(self):
        ...
        parafrasis = self.parafrasis_text.get("1.0", tk.END).strip()
        if parafrasis:
            ejercicio_actual = self.ejercicios[self.ejercicios_completados]
            self.parafrasis_guardadas.append({
    ...
    def guardar_parafrasis(self):
        """Guarda todas las paráfrasis en un archivo de texto."""
        nombre_archivo = "parafrasis_ciencias_11_tkinter.txt"
        try:
            with open(nombre_archivo, "w", encoding="utf-8") as archivo:
                for item in self.parafrasis_guardadas:
                    archivo.write(f"--- Ejercicio {item['numero']} - {item['titulo']} ---\n")
                    archivo.write(f"{item['parafrasis']}\n\n")
