def mostrar_reglas():
    reglas = [
        ...
        "6. Se escriben con 'C' los verbos terminados en '-acer', '-ecer', '-ocer' y '-ucir': nacer, crecer, conocer, producir.",
        "7. Se escriben con 'C' los adjetivos que terminan en '-áceo', '-áceas' y '-áceos': violáceo, crustáceas, crustáceos.",
    ]
    for regla in reglas:
        print(regla)
...
def practicar_reglas():
    ...
    for i, regla in enumerate(reglas, 1):
        ...
        if regla_valida(respuesta, regla):
...
def regla_valida(respuesta, regla):
    # Elimina espacios y convierte a minúsculas para comparar
    return respuesta.strip().lower() == regla.lower()
