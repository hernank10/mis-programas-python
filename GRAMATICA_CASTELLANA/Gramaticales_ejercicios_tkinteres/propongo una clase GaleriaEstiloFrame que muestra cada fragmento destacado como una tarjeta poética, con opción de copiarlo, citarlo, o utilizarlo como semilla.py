...
class GaleriaEstiloFrame(tk.Frame):
    def __init__(self, master, controller=None):
        ...
        tk.Label(self, text="🌟 Galería de Estilo", font=("Georgia", 20), bg="#f0f8f5").pack(pady=20)

        self.marco = tk.Canvas(self, bg="#f0f8f5")
        self.scroll = tk.Scrollbar(self, orient="vertical", command=self.marco.yview)
        self.frame_tarjetas = tk.Frame(self.marco, bg="#f0f8f5")
    ...
    def cargar_tarjetas(self):
        ...
        if not os.path.exists(archivo):
            tk.Label(self.frame_tarjetas, text="Aún no hay fragmentos destacados.", bg="#f0f8f5", font=("Georgia", 12)).pack(pady=10)
            return
        ...
        with open(archivo, "r", encoding="utf-8") as f:
            try:
                ...
                tk.Label(self.frame_tarjetas, text="Error al cargar la galería.", bg="#f0f8f5").pack()
                return
        ...
        for entrada in datos:
            ...
            btn = tk.Button(marco, text="📋 Copiar", command=lambda t=entrada["contenido"]: self.copiar_fragmento(t))
            btn.pack(pady=2)
