...
voices = {
    "Inglés": "com.apple.speech.synthesis.voice.Alex",
    "Portugués": "com.apple.speech.synthesis.voice.Luciana",
    "Hebreo": "com.apple.speech.synthesis.voice.Carmit",
    "Latín": "com.apple.speech.synthesis.voice.Moira",
    "Griego": "com.apple.speech.synthesis.voice.Sophia"
}
...
def start_exercise(language):
    ...
    tk.Label(exercise_window, text=f"Conjuga el verbo: {verb}", font=("Arial", 14)).pack(pady=10)
    ...
    def pronounce_verb():
        ...
        engine.say(f"The verb is {verb}")
        for form in conjugation:
            engine.say(form)
        engine.runAndWait()

    tk.Button(exercise_window, text="Escuchar Pronunciación", command=pronounce_verb).pack(pady=10)
...
root = tk.Tk()
root.title("Ejercicios de Conjugación y Pronunciación")
