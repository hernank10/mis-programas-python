...
def show_story():
    ...
    for line in story_text.split('\n'):
        print(line)
        time.sleep(0.05) # Pausa reducida para flujo más rápido en consola
...
def game_listen_and_repeat():
    ...
    for i, p in enumerate(phrases[:3]): # Solo 3 para no alargar demasiado
        ...
        time.sleep(1) # Pausa para "escuchar"
...
def create_own_example():
    """Permite al usuario crear su propio ejemplo de comunicación."""
    show_header("✨ ¡Crea Tu Propio Ejemplo! ✨")
    print("Piensa en una situación donde quieras escuchar bien, pensar antes de hablar o hablar con claridad y respeto.\n")
    print("Vamos a crear tu propio ejemplo. Responde a las siguientes preguntas:\n")

    scenario = input("1. Imagina una situación: ¿Qué pasó o qué quieres decir? (Ej: 'Quiero un juguete que tiene mi amigo')\nTu situación: ").strip()
    if not scenario:
        print("Necesitas escribir una situación para tu ejemplo.")
        input("Presiona ENTER para volver al menú...")
        return

    good_response = input("\n2. ¿Cómo responderías o actuarías tú de forma CLARA y RESPETUOSA en esa situación? (Ej: '¿Me prestas tu juguete, por favor?')\nTu buena respuesta: ").strip()
    if not good_response:
        print("Necesitas escribir una buena respuesta.")
        input("Presiona ENTER para volver al menú...")
        return

    bad_response = input("\n3. ¿Cómo NO deberías responder o actuar (mal ejemplo)? (Ej: '¡Dame ese juguete ahora mismo!')\nTu mal ejemplo: ").strip()
    if not bad_response:
        print("Necesitas escribir un mal ejemplo.")
        input("Presiona ENTER para volver al menú...")
        return

    print("\n--- ¡Tu Ejemplo Creado! ---")
    print(f"Situación: {scenario}")
    print(f"Lo harías bien si dices/haces: '{good_response}' ✅")
    print(f"NO lo harías bien si dices/haces: '{bad_response}' ❌")
    print("\n¡Qué buen ejemplo has creado! ¡Eso muestra que estás aprendiendo mucho! 🎉")
    input("Presiona ENTER para volver al menú principal...")
...
def show_main_menu():
    ...
    while True:
        ...
        print("6. ✨ ¡Crea Tu Propio Ejemplo! ✨") # Nueva opción
        print("7. Salir de la Lección")
        print("*" * 50)

        choice = input("Elige un número (1-7): ") # Opciones actualizadas
