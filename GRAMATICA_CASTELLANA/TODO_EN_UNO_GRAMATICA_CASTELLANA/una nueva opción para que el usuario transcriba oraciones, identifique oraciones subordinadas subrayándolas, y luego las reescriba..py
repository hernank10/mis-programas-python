...
def mostrar_menu():
    ...
    print("7. Transcribir y analizar oraciones")
    print("8. Salir")
...
def guardar_en_archivo():
    ...
    with open(archivo, "w") as file:
        ...
        for tipo, ejemplo in oraciones_subordinadas.items():
            file.write(f"{tipo}: {ejemplo}\n")
    print(f"Ejercicios guardados en {archivo}")
    input("\nPresiona Enter para volver al menú.")

# Transcribir y analizar oraciones
def transcribir_analizar():
    print("\n--- Transcribir y Analizar Oraciones ---")
    print("Escribe hasta 10 oraciones. Identifica la oración subordinada subrayándola con guiones bajos (_).")
    print("Por ejemplo: No iremos al parque porque _está lloviendo_.")
    print("Escribe 'FIN' para terminar antes de completar las 10 oraciones.")

    oraciones_usuario = []
    for i in range(10):
        oracion = input(f"Oración {i+1}: ")
        if oracion.upper() == "FIN":
            break
        oraciones_usuario.append(oracion)
    
    # Guardar oraciones en un archivo
    archivo = "oraciones_transcritas.txt"
    with open(archivo, "w") as file:
        file.write("--- Oraciones Transcritas y Analizadas ---\n")
        for oracion in oraciones_usuario:
            file.write(oracion + "\n")
    print(f"Oraciones guardadas en {archivo}")
    input("\nPresiona Enter para volver al menú.")
