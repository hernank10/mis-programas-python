...
class AdvancedArgumentationApp:
    def __init__(self, master):
        ...
        # Geometría y Centrado de Ventana
        ...
        master.config(bg="#F0F8FF") # Color de Fondo General (AliceBlue)
    ...
    def clear_frame(self):
        for widget in self.master.winfo_children():
            # Mantener la barra de menú, destruir solo otros widgets
            if widget != self.master.winfo_children()[0]: 
                widget.destroy()
        self.master.config(bg="#F0F8FF") # Asegurar que el fondo se mantenga

    # --- Nueva función para crear botones de navegación ---
    def create_navigation_buttons(self):
        nav_frame = tk.Frame(self.master, bg="#F0F8FF", pady=10)
        nav_frame.pack(side=tk.BOTTOM, fill=tk.X)

        btn_font = ("Arial", 11)
        btn_bg = "#E0FFFF" # LightCyan
        btn_fg = "#333333"

        tk.Button(nav_frame, text="📚 Teoría", command=self.show_theory, font=btn_font, bg=btn_bg, fg=btn_fg).pack(side=tk.LEFT, padx=5, pady=5)
        tk.Button(nav_frame, text="💡 Ejemplos", command=self.show_examples, font=btn_font, bg=btn_bg, fg=btn_fg).pack(side=tk.LEFT, padx=5, pady=5)
        tk.Button(nav_frame, text="📝 Ejercicios", command=self.start_exercises, font=btn_font, bg=btn_bg, fg=btn_fg).pack(side=tk.LEFT, padx=5, pady=5)
        tk.Button(nav_frame, text="✨ Gen. Ejemplos", command=self.show_random_examples, font=btn_font, bg=btn_bg, fg=btn_fg).pack(side=tk.LEFT, padx=5, pady=5)
        tk.Button(nav_frame, text="🏠 Menú Principal", command=self.show_main_page, font=btn_font, bg="#ADD8E6", fg="#333333").pack(side=tk.RIGHT, padx=5, pady=5) # LightBlue

    def show_main_page(self):
        self.clear_frame()
        
        main_label = tk.Label(self.master, text="¡Bienvenido a la Lección 6!\nArgumentación Avanzada y Lógica (Preuniversitario)", 
                              font=("Arial", 20, "bold"), pady=20, bg="#F0F8FF", fg="#2F4F4F") 
        main_label.pack()

        info_label = tk.Label(self.master, text="Usa el menú 'Lección' o los botones de navegación para explorar:\n\n"
                                               ...
                                               font=("Arial", 13), justify=tk.LEFT, bg="#F0F8FF", fg="#36454F")
        ...
        self.create_navigation_buttons() # Add navigation buttons to main page too

    def show_theory(self):
        self.clear_frame()
        
        title_label = tk.Label(self.master, text="🧠 TEORÍA: ARGUMENTACIÓN AVANZADA Y LÓGICA 🔬", font=("Arial", 17, "bold"), pady=10, bg="#F0F8FF", fg="#004d40") 
        title_label.pack()
...
"""
        ...
        self.create_navigation_buttons() # Add navigation buttons
...
"""
    ...
    def display_exercise(self):
        ...
        arg_label = tk.Label(self.master, text=f"Argumento: \"{exercise['arg']}\"", wraplength=850, justify=tk.LEFT, 
                             font=("Arial", 12), fg="#00008B", bg="#F0F8FF") 
        ...
        # Mostrar/ocultar frames relevantes según la selección del tipo
    ...
    def check_exercise(self):
        ...
        is_type_correct = (selected_type == exercise["tipo_esperado"])
        if is_type_correct:
        ...
        is_current_exercise_correct = is_type_correct # Start with type correctness for overall exercise score
        ...
        if selected_type == "DEDUCTIVO":
            ...
            is_validity_correct = (selected_validity == exercise["validez_esperada"])
            if is_validity_correct:
                feedback_message += "✅ Validez: Correcta.\n"
            else:
                feedback_message += f"❌ Validez: Incorrecta. Era {exercise['validez_esperada']}.\n"
                is_current_exercise_correct = False # If validity is wrong, whole exercise is wrong

            if exercise["validez_esperada"] == "VÁLIDO": # Solo se pide solidez si el argumento es válido
                if not selected_soundness:
                    messagebox.showwarning("Atención", "Por favor, selecciona si es SÓLIDO.")
                    return

                is_soundness_correct = (selected_soundness == exercise["solidez_esperada"])
                if is_soundness_correct:
                    feedback_message += "✅ Solidez: Correcta.\n"
                else:
                    feedback_message += f"❌ Solidez: Incorrecta. Era {exercise['solidez_esperada']}.\n"
                    is_current_exercise_correct = False # If soundness is wrong, whole exercise is wrong
            else: # Si es inválido, no puede ser sólido. El usuario no debería marcar sólido.
                feedback_message += "   (Al ser inválido, no puede ser sólido. Su respuesta de solidez no se evalúa.)\n"
                # If the user correctly identified it as invalid, then they effectively "got" the soundness part right by implication
                # If they tried to select "solid" for an invalid arg, the validity check already fails them.
            ...
            is_strength_correct = (selected_strength == exercise["fuerza_esperada"])
            if is_strength_correct:
                feedback_message += "✅ Fuerza inductiva: Correcta.\n"
            else:
                feedback_message += f"❌ Fuerza inductiva: Incorrecta. Era {exercise['fuerza_esperada']}.\n"
                is_current_exercise_correct = False
        
        if is_current_exercise_correct:
            self.score += 1
    ...
    def show_exercise_results(self):
        ...
        self.create_navigation_buttons() # Add navigation buttons to results page
    ...
    def show_random_examples(self):
        ...
        random_examples_text.config(state=tk.DISABLED) 
        self.create_navigation_buttons() # Add navigation buttons
