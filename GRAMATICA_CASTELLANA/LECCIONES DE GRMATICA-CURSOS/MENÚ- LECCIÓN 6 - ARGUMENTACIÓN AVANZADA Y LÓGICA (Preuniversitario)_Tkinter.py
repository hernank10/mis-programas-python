...
class AdvancedArgumentationApp:
    def __init__(self, master):
        ...
        master.geometry("950x750") # Larger window for content
    ...
    def show_theory(self):
        ...
        title_label = tk.Label(self.master, text="🧠 TEORÍA: ARGUMENTACIÓN AVANZADA Y LÓGICA 🔬", font=("Arial", 17, "bold"), pady=10)
        title_label.pack()

        theory_text = scrolledtext.ScrolledText(self.master, wrap=tk.WORD, font=("Arial", 12), width=100, height=32)
...
"""
    ...
    def show_examples(self):
        ...
        title_label = tk.Label(self.master, text="💡 EJEMPLOS DE VALIDEZ, SOLIDEZ Y TIPOS DE RAZONAMIENTO 💡", font=("Arial", 16, "bold"), pady=10)
        title_label.pack()

        examples_text = scrolledtext.ScrolledText(self.master, wrap=tk.WORD, font=("Arial", 12), width=100, height=32)
...
"""
    ...
    def display_exercise(self):
        ...
        # --- Pregunta de Tipo ---
        tk.Label(self.master, text="1. ¿Qué tipo de razonamiento es?", font=("Arial", 12)).pack(pady=(10,0))
        self.type_var = tk.StringVar(self.master)
        self.type_var.set("")
        tk.Radiobutton(self.master, text="DEDUCTIVO", variable=self.type_var, value="DEDUCTIVO", font=("Arial", 11)).pack(anchor=tk.W, padx=50)
        tk.Radiobutton(self.master, text="INDUCTIVO", variable=self.type_var, value="INDUCTIVO", font=("Arial", 11)).pack(anchor=tk.W, padx=50)

        # --- Preguntas de Validez/Solidez/Fuerza (inicialmente ocultas) ---
        self.deductive_questions_frame = tk.Frame(self.master)
        self.inductive_questions_frame = tk.Frame(self.master)
        
        # Deductivo: Validez y Solidez
        tk.Label(self.deductive_questions_frame, text="2. Si es deductivo, ¿es VÁLIDO?", font=("Arial", 12)).pack(pady=(10,0), anchor=tk.W)
        self.validity_var = tk.StringVar(self.deductive_questions_frame)
        self.validity_var.set("")
        tk.Radiobutton(self.deductive_questions_frame, text="SÍ", variable=self.validity_var, value="VÁLIDO", font=("Arial", 11)).pack(anchor=tk.W)
        tk.Radiobutton(self.deductive_questions_frame, text="NO (INVÁLIDO)", variable=self.validity_var, value="INVÁLIDO", font=("Arial", 11)).pack(anchor=tk.W)

        tk.Label(self.deductive_questions_frame, text="3. Si es válido, ¿es SÓLIDO? (Todas las premisas son verdaderas)", font=("Arial", 12)).pack(pady=(10,0), anchor=tk.W)
        self.soundness_var = tk.StringVar(self.deductive_questions_frame)
        self.soundness_var.set("")
        tk.Radiobutton(self.deductive_questions_frame, text="SÍ (SÓLIDO)", variable=self.soundness_var, value="SÓLIDO", font=("Arial", 11)).pack(anchor=tk.W)
        tk.Radiobutton(self.deductive_questions_frame, text="NO (NO SÓLIDO)", variable=self.soundness_var, value="NO SÓLIDO", font=("Arial", 11)).pack(anchor=tk.W)

        # Inductivo: Fuerza
        tk.Label(self.inductive_questions_frame, text="2. Si es inductivo, ¿es FUERTE o DÉBIL?", font=("Arial", 12)).pack(pady=(10,0), anchor=tk.W)
        self.strength_var = tk.StringVar(self.inductive_questions_frame)
        self.strength_var.set("")
        tk.Radiobutton(self.inductive_questions_frame, text="FUERTE", variable=self.strength_var, value="FUERTE", font=("Arial", 11)).pack(anchor=tk.W)
        tk.Radiobutton(self.inductive_questions_frame, text="DÉBIL", variable=self.strength_var, value="DÉBIL", font=("Arial", 11)).pack(anchor=tk.W)

        # Show/hide relevant frames based on type selection
        self.type_var.trace_add("write", self.toggle_question_frames)
        # Call initially in case an option is pre-selected (not likely with set(""))
    ...
    def check_exercise(self):
        ...
        if selected_type == "DEDUCTIVO":
            ...
            if exercise["validez_esperada"] == "VÁLIDO":
                ...
                selected_soundness = "NO SÓLIDO" # Consider it correct for this logic path.
    ...
    def show_exercise_results(self):
        ...
        results_label = tk.Label(self.master, text="--- ¡FIN DE LOS EJERCICIOS DE EVALUACIÓN! ---", font=("Arial", 18, "bold"), pady=10)
        results_label.pack()

        score_label = tk.Label(self.master, text=f"Tu puntuación final es: {self.score}/{self.total_exercises}!", font=("Arial", 15), pady=10)
        score_label.pack()
        ...
        feedback_label = tk.Label(self.master, text=message, font=("Arial", 13), pady=10)
        feedback_label.pack()
    ...
    def show_random_examples(self):
        ...
        title_label = tk.Label(self.master, text="✨ GENERADOR DE EJEMPLOS ALEATORIOS ✨", font=("Arial", 16, "bold"), pady=10)
        title_label.pack()

        random_examples_text = scrolledtext.ScrolledText(self.master, wrap=tk.WORD, font=("Arial", 12), width=100, height=32)
