...
# ==============================================================================
# Define el tamaño de la ventana para una experiencia inicial más amplia.
Window.size = (800, 600)

# ==============================================================================
# DATOS DEL QUIZ
# ==============================================================================
# La base de datos de preguntas y respuestas, ahora para la GUI de Kivy.
# Se mantiene la misma estructura para la compatibilidad.
...
# ==============================================================================
class LenguasIndigenasQuizApp(App):
    """
    Aplicación de Quiz de Lenguas Indígenas de Colombia con Kivy.
    """
    def build(self):
        """
        Método principal que construye y devuelve el widget raíz de la aplicación.
        """
        # Se inicializan las variables del quiz
        self.questions = random.sample(EJERCICIOS_LENGUAS_INDIGENAS, k=len(EJERCICIOS_LENGUAS_INDIGENAS))
        self.current_question_index = 0
        self.score = 0
        self.total_questions = len(self.questions)

        # Se configura el layout principal. Un BoxLayout vertical es ideal para esto.
        self.root = BoxLayout(orientation='vertical', padding=dp(20), spacing=dp(10))
        self.root.background_color = get_color_from_hex('#f0f0f0')

        # Título del quiz
        title_label = Label(
            text="Quiz: Lenguas Indígenas de Colombia",
            font_size=dp(24),
            bold=True,
            color=get_color_from_hex('#4a4a4a'),
            size_hint_y=0.1
        )
        ...
        self.question_label = Label(
            ...
            valign='middle',
            size_hint_y=0.2,
            text_size=(self.root.width * 0.9, None),  # Ancho del texto
        ...
        self.root.add_widget(self.answer_entry)
        # Bindeamos la tecla Enter para que llame a la función check_answer
        self.answer_entry.bind(on_text_validate=self.check_answer)

        # Etiqueta para mostrar la puntuación y el estado
        ...
        self.root.add_widget(self.feedback_label)
    ...
    def check_answer(self, instance):
        """
        Comprueba la respuesta del usuario, actualiza el feedback y habilita el botón 'Siguiente'.
        """
        ...
        if is_correct:
            ...
            self.feedback_label.color = get_color_from_hex('#2e7d32')  # Verde
        else:
            self.feedback_label.text = "Incorrecto. ❌"
            self.feedback_label.color = get_color_from_hex('#c62828')  # Rojo

        self.explanation_label.text = f"[b]Respuesta correcta:[/b] {correct_answer.capitalize()}\n[b]Explicación:[/b] {current_question['explicacion']}"
        self.explanation_label.markup = True

        self.check_button.disabled = True
        self.next_button.disabled = False
        self.answer_entry.disabled = True
    ...
    def display_question(self):
        """
        Muestra la pregunta actual y reinicia los widgets para la siguiente ronda.
        """
        current_question = self.questions[self.current_question_index]
        self.question_label.text = current_question["pregunta"]
        self.status_label.text = f"Pregunta {self.current_question_index + 1} de {self.total_questions} | Puntuación: {self.score}"
        ...
        self.answer_entry.disabled = False
        self.answer_entry.focus = True

    def show_final_results(self):
        """
        Limpia la pantalla y muestra los resultados finales con un botón para reiniciar.
        """
        ...
        self.root.add_widget(final_score_label)

        result_text = f"Has obtenido [b]{self.score}[/b] de [b]{self.total_questions}[/b] respuestas correctas."
        result_label = Label(
            text=result_text,
            font_size=dp(20),
            markup=True,
