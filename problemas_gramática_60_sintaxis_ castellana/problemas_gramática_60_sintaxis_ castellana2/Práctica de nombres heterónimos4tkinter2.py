...
def practicar_completacion():
    ...
    def verificar_respuesta():
        ...
        if respuesta == palabra_femenina:
            ...
            messagebox.showerror("Incorrecto", f"✖ La respuesta correcta era '{palabra_femenina}'.")
        ventana_completacion.destroy()
    
    ventana_completacion = tk.Toplevel(root)
    ventana_completacion.title("Ejercicio de Completación")
    tk.Label(ventana_completacion, text=f"Mi {palabra_masculina} y mi ____ me enseñaron grandes valores.").pack()
    entrada = tk.Entry(ventana_completacion)
    entrada.pack()
    tk.Button(ventana_completacion, text="Verificar", command=verificar_respuesta).pack()
