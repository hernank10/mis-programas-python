...
def cuestionario():
    preguntas = [
        ("¿Qué estructura indica una acción previa?", ["Con", "Después de", "Antes de", "Por"], "Antes de"),
        ("¿Qué estructura indica una acción simultánea?", ["Con", "Al", "Después de", "Por"], "Al"),
        ("¿Qué estructura puede indicar causa?", ["Antes de", "Después de", "Por", "Al"], "Por"),
        ("¿Qué estructura expresa una acción acompañada de otra?", ["Con", "Después de", "Antes de", "Por"], "Con"),
    ]
    
    correctas = 0
    for pregunta, opciones, correcta in preguntas:
        respuesta = simpledialog.askstring("Cuestionario", f"{pregunta}\nOpciones: {', '.join(opciones)}")
        if respuesta and respuesta.strip() == correcta:
            correctas += 1
    
    messagebox.showinfo("Resultados", f"Has respondido correctamente {correctas} de {len(preguntas)} preguntas.")
...
def iniciar_interfaz():
    ...
    tk.Button(root, text="Cuestionario", command=cuestionario).pack(fill="x", padx=20, pady=5)
