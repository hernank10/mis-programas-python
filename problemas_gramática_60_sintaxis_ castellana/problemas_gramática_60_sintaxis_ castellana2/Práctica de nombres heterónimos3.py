...
def practicar_completacion():
    while True:
        ...
        print(oracion)
        respuesta = input("Completa la oración (o escribe 'salir' para volver al menú): ").strip().lower()
        
        if respuesta == "salir":
            break
        elif respuesta == palabra_femenina:
...
def practicar_redaccion():
    while len(ejemplos_usuario) < 100:
        ...
        oracion1 = input("Tu oración (o escribe 'salir' para volver al menú): ")
        if oracion1.lower() == "salir":
            break
