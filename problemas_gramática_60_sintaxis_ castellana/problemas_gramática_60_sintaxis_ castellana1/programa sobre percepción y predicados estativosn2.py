...
class Ejercicios:
    ...
    def realizar_ejercicios(self):
        ...
        for ejercicio in self.redaccion:
            print(ejercicio)
            if "percepción no epistémica" in ejercicio:
                print("Ejemplo: Juan vio correr a su perro en el parque.")
            oracion_usuario = input("Escribe tu oración: ")
            if "percepción no epistémica" in ejercicio:
                if len(self.ejemplos_guardados) < 100:
                    self.ejemplos_guardados.append(oracion_usuario)
                    print("Oración guardada con éxito!\n")
                else:
                    print("Has alcanzado el límite de 100 ejemplos guardados.\n")
        
        if self.ejemplos_guardados:
            print("\nOraciones guardadas:")
            for idx, oracion in enumerate(self.ejemplos_guardados, 1):
                print(f"{idx}. {oracion}")
