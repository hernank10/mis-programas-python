...
def ejercicio_1():
    ...
    for i, oracion in enumerate(oraciones):  # Corrección de typo: "orasiones" → "oraciones"
        print(f"\nOración {i+1}: {oracion}")
...
def ejercicio_4():
    print("""
    EJERCICIO 4: REDACTAR ORACIONES
    -------------------------------
    A continuación, se te darán indicaciones para redactar oraciones con verbos de percepción. Utiliza la forma no finita indicada (infinitivo, gerundio o participio) en cada caso.
    """)

    indicaciones = [
        "Redacta una oración con el verbo 'ver' y un infinitivo.",
        "Redacta una oración con el verbo 'oír' y un gerundio.",
        "Redacta una oración con el verbo 'encontrar' y un participio.",
        "Redacta una oración con el verbo 'sentir' y un infinitivo.",
        "Redacta una oración con el verbo 'observar' y un gerundio."
    ]

    for i, indicacion in enumerate(indicaciones):
        print(f"\nIndicación {i+1}: {indicacion}")
        oracion = input("Escribe tu oración: ")
        print(f"¡Bien hecho! Tu oración es: '{oracion}'.")

def ejercicio_5():
    print("""
    EJERCICIO 5: IDENTIFICAR Y CORREGIR ERRORES
    -------------------------------------------
    A continuación, se te presentarán oraciones con errores en el uso de las formas no finitas (infinitivo, gerundio o participio). Identifica el error y corrige la oración.
    """)

    oraciones_con_errores = [
        "La vi cantado en el escenario.",  # Error: debería ser "cantar" o "cantando".
        "Los escuché tocado la guitarra.",  # Error: debería ser "tocar" o "tocando".
        "Encontré el libro rompiendo.",  # Error: debería ser "roto".
        "Sentí el agua mojado mis pies.",  # Error: debería ser "mojar" o "mojando".
        "Observé al perro escapado del jardín."  # Error: debería ser "escapar" o "escapando".
    ]

    correcciones = [
        "La vi cantar en el escenario.",  # Corrección con infinitivo.
        "Los escuché tocar la guitarra.",  # Corrección con infinitivo.
        "Encontré el libro roto.",  # Corrección con participio.
        "Sentí el agua mojar mis pies.",  # Corrección con infinitivo.
        "Observé al perro escapar del jardín."  # Corrección con infinitivo.
    ]

    for i, oracion in enumerate(oraciones_con_errores):  # Corrección de typo: "orasiones_con_errores" → "oraciones_con_errores"
        print(f"\nOración {i+1}: {oracion}")
        correccion_usuario = input("Corrige la oración: ")
        if correccion_usuario.lower() == correcciones[i].lower():
            print("¡Correcto! La oración corregida es: '{}'.".format(correcciones[i]))
        else:
...
def main():
    ...
    while True:
        ...
        print("4. Redactar oraciones.")
        print("5. Identificar y corregir errores.")
        print("6. Salir.")
