# Este programa crea un juego educativo de adjetivos y pronombres usando Pygame,
# incorporando efectos visuales avanzados y un enfoque de gamificación.
# Sirve como una demostración de los conceptos del Módulo 3 y más.
...
pygame.display.set_caption("Juego de Adjetivos y Pronombres")
...
# --- Contenido de la lección (Adjetivos y Pronombres) ---
lecciones = [
    ("grande", "big", "adjetivo"),
    ("pequeño", "small", "adjetivo"),
    ("feliz", "happy", "adjetivo"),
    ("triste", "sad", "adjetivo"),
    ("rápido", "fast", "adjetivo"),
    ("lento", "slow", "adjetivo"),
    ("yo", "I", "pronombre"),
    ("tú", "you", "pronombre"),
    ("él", "he", "pronombre"),
    ("ella", "she", "pronombre"),
    ("nosotros", "we", "pronombre"),
    ("ellos", "they", "pronombre")
]
...
class SistemaParticulas:
    ...
    def __init__(self):
        self.particulas = []
        # Emoticones de sintaxis bilingüe para estudiantes de nivel preuniversitario.
...
def iniciar_juego():
    ...
    global estado_juego, puntaje, palabras_jugadas, pregunta_actual, respuesta_usuario
...
def obtener_nueva_pregunta():
    ...
    preguntas_disponibles = [p for p in lecciones if p not in palabras_jugadas]
    if preguntas_disponibles:
        pregunta_actual = random.choice(preguntas_disponibles)
        palabras_jugadas.append(pregunta_actual)
    else:
        ir_a_resultados()
...
while en_ejecucion:
    ...
    if estado_juego == "menu":
        titulo_render = FUENTE_TITULO.render("Aprende Adjetivos y Pronombres", True, BLANCO)
        ...
        # Tipo de palabra
        tipo_render = FUENTE_PEQUENA.render(f"Tipo: {pregunta_actual[2]}", True, BLANCO)
        tipo_rect = tipo_render.get_rect(center=(ANCHO // 2, ALTO // 2 - 100))
        PANTALLA.blit(tipo_render, tipo_rect)
        
        # Palabra a traducir
        palabra_render = FUENTE_NORMAL.render(pregunta_actual[0], True, BLANCO)
        ...
        final_render = FUENTE_NORMAL.render(f"Puntaje final: {puntaje} de {len(lecciones)}", True, BLANCO)
