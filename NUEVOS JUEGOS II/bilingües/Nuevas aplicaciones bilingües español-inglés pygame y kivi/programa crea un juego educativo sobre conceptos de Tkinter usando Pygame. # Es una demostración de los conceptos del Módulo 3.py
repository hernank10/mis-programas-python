# Este programa crea un juego educativo sobre conceptos de Tkinter usando Pygame.
# Es una demostración de los conceptos del Módulo 3, incorporando todos los
# elementos visuales avanzados solicitados.
...
pygame.display.set_caption("Juego de Conceptos de Tkinter")
...
# --- Contenido de la lección (conceptos de Tkinter) ---
# Cada tupla contiene: (pregunta, respuesta_correcta, tipo_de_widget)
lecciones_tkinter = [
    ("¿Qué widget se usa para mostrar texto estático o imágenes?", "Label", "widget"),
    ("¿Qué widget permite al usuario interactuar con la app, ejecutando una acción?", "Button", "widget"),
    ("¿Qué widget se utiliza para obtener una línea de texto del usuario?", "Entry", "widget"),
    ("¿Qué widget se usa para agrupar otros widgets o darles un borde?", "Frame", "widget"),
    ("¿Qué función muestra un cuadro de diálogo de advertencia simple?", "messagebox", "función"),
    ("¿Qué función inicia el bucle principal de la aplicación?", "mainloop", "función"),
    ("¿Qué método se usa para empaquetar y posicionar un widget?", "pack", "método"),
    ("¿Qué método se usa para posicionar un widget en una cuadrícula?", "grid", "método")
]
...
class SistemaParticulas:
    ...
    def __init__(self):
        self.particulas = []
        # Emoticones de sintaxis de Python/Tkinter para estudiantes preuniversitarios
...
def obtener_nueva_pregunta():
    """Selecciona una nueva pregunta que no se haya usado."""
    global pregunta_actual
    preguntas_disponibles = [p for p in lecciones_tkinter if p not in preguntas_jugadas]
...
def verificar_respuesta():
    ...
    if respuesta_usuario.lower().strip() == respuesta_correcta.lower().strip():
        mensaje_feedback = f"¡Correcto! Es un {pregunta_actual[2]} ✅✨"
...
while en_ejecucion:
    ...
    if estado_juego == "menu":
        titulo_render = FUENTE_TITULO.render("Aprende sobre Tkinter", True, BLANCO)
        ...
        idioma_es_render = FUENTE_PEQUENA.render("Pregunta de Tkinter 💻", True, BLANCO)
        ...
        final_render = FUENTE_NORMAL.render(f"Puntaje final: {puntaje} de {len(lecciones_tkinter)}", True, BLANCO)
