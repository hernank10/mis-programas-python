# Este programa implementa el Módulo 9 del plan de aprendizaje,
# adaptado a Kivy, para un diseño de interfaz de usuario completo y multiplataforma.
# Incluye la integración de un modelo de lenguaje de IA para generar preguntas dinámicas.
...
class JuegoScreen(BaseScreen):
    ...
    async def obtener_pregunta_ia(self):
        ...
        try:
            ...
            response = await kivy.network.urlrequest.UrlRequest(
                api_url,
                req_body=json.dumps(payload),
                req_headers={"Content-Type": "application/json"},
                on_success=self.procesar_respuesta_ia,
                on_failure=self.manejar_error_ia,
                on_error=self.manejar_error_ia
            )

        except Exception as e:
            print(f"Error al llamar a la IA: {e}")
            self.obtener_pregunta_db()

    def procesar_respuesta_ia(self, request, result):
        """Procesa la respuesta exitosa de la IA."""
        try:
            # La respuesta viene como un string, necesitamos parsearla
            ...
            self.actualizar_ui_con_pregunta()
        except (KeyError, json.JSONDecodeError) as e:
            print(f"Error al procesar la respuesta de la IA: {e}")
            self.obtener_pregunta_db()

    def manejar_error_ia(self, request, result):
        """Maneja los errores de la llamada a la IA."""
        print(f"Error en la llamada a la IA: {result}")
        self.obtener_pregunta_db()

    def obtener_pregunta_db(self):
        """Obtiene una pregunta de la base de datos local."""
        conn = sqlite3.connect(DB_NAME)
        cursor = conn.cursor()
    ...
    def actualizar_ui_con_pregunta(self):
        """Actualiza la interfaz de usuario con la nueva pregunta."""
        self.ids.loading_label.opacity = 0
        self.ids.oracion_label.opacity = 1
        self.ids.transformacion_label.opacity = 1
