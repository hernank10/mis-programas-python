# Este programa implementa un juego de aventura en Pygame para el Módulo 10,
# añadiendo una interfaz de usuario completa con menús y botones.
...
def obtener_pregunta_ia(dificultad, tipo_oracion=None):
    ...
    try:
        prompt_ia = f"Genera una pregunta de sintaxis en formato JSON. La pregunta debe incluir 'oracion_original' (una oración en inglés), 'oracion_transformada' (la traducción equivalente en español), 'tipo_transformacion' (la transformación sintáctica, una de 'adjetiva', 'sustantiva', 'causal', 'consecutiva', 'temporal', 'comparativa', 'concesiva', 'final', o 'condicional'), y 'idioma' ('inglés' o 'español'). La transformación debe ser del tipo '{tipo_oracion}'."
...
class Game:
    ...
    def handle_input(self):
        """Maneja las entradas del usuario (teclado y clics)."""
    ...
    def draw_background(self):
        """Dibuja el fondo animado con partículas."""
    ...
    def draw_menu(self):
        ...
        title_text = font_large.render("Aventura Gramatical", True, WHITE)
        title_rect = title_text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 4))
        SCREEN.blit(title_text, title_rect)
        
        # Botón de Jugar
        self.play_button_rect = pygame.Rect(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2, 200, 50)
        pygame.draw.rect(SCREEN, BUTTON_COLOR, self.play_button_rect, border_radius=10)
        play_text = font_medium.render("Jugar", True, WHITE)
        play_text_rect = play_text.get_rect(center=self.play_button_rect.center)
        SCREEN.blit(play_text, play_text_rect)

        # Botón de Salir
        self.exit_button_rect = pygame.Rect(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2 + 70, 200, 50)
        pygame.draw.rect(SCREEN, BUTTON_COLOR, self.exit_button_rect, border_radius=10)
        exit_text = font_medium.render("Salir", True, WHITE)
    ...
    def draw_pause_menu(self):
        ...
        pause_text = font_large.render("PAUSA", True, WHITE)
        pause_rect = pause_text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 4))
        SCREEN.blit(pause_text, pause_rect)

        self.resume_button_rect = pygame.Rect(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2, 200, 50)
        pygame.draw.rect(SCREEN, BUTTON_COLOR, self.resume_button_rect, border_radius=10)
        resume_text = font_medium.render("Reanudar", True, WHITE)
        resume_text_rect = resume_text.get_rect(center=self.resume_button_rect.center)
        SCREEN.blit(resume_text, resume_text_rect)

        self.menu_button_rect = pygame.Rect(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2 + 70, 200, 50)
        pygame.draw.rect(SCREEN, BUTTON_COLOR, self.menu_button_rect, border_radius=10)
        menu_text = font_medium.render("Menú Principal", True, WHITE)
    ...
    def run(self):
        ...
        while True:
            ...
            if self.state == STATE_MENU:
                ...
                self.submit_button_rect = pygame.Rect(SCREEN_WIDTH // 2 - 100, 350, 200, 50)
                pygame.draw.rect(SCREEN, BUTTON_COLOR, self.submit_button_rect, border_radius=10)
