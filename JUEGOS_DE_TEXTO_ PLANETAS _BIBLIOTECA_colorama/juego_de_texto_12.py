...
class Jugador:
    ...
    def mostrar_estado(self):
        ...
        print(
            f"{Fore.WHITE if COLOR_ENABLED else ''}Equipo:{Style.RESET_ALL} {', '.join(self.equipo) if self.equipo else 'Ninguno'}")
        print(
            f"{Fore.WHITE if COLOR_ENABLED else ''}Inventario:{Style.RESET_ALL} {', '.join(self.inventario) if self.inventario else 'Vacío'}")
        print("-" * 30 + "\n")
...
class Personaje:
    ...
    def hablar(self, accion):
        if accion in self.dialogos:
            print(
                f"{Fore.GREEN if COLOR_ENABLED else ''}{self.nombre}{Style.RESET_ALL} dice: '{self.dialogos[accion]}'")
...
class Juego:
    ...
    def setup_juego(self):
        ...
        self.mapa["Planeta Azul"] = Escenario(
            ...
            conexiones={'este': 'Playa', 'oeste': 'Ciudad Flotante', 'norte': 'Base Espacial'},
            objetos=['conchas']
        )
    ...
    def acerca_de(self, argumentos=None):
        ...
        print(
            "Proporciona una experiencia interactiva donde el jugador debe explorar, recolectar objetos y resolver acertijos para reparar su nave espacial.")
    ...
    def iniciar_juego(self):
        ...
        while True:
            ...
            if self.estado["nave_reparada"]:
                print(
                    Fore.BLUE + "¡Felicidades! Has completado la misión y reparado tu nave espacial." + Style.RESET_ALL)
                sys.exit()

            # Lógica adicional para detectar cuando el jugador ha recolectado todos los objetos
            # Por ejemplo, en la ubicación "Sala Secreta", después de recolectar la llave completa
