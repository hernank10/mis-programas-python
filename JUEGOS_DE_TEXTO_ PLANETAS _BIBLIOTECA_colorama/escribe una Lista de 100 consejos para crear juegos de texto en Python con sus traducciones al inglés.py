...
def mostrar_menu():
    ...
    print("5. Reescribir los consejos del 1 al 100")
    print("6. Practicar la escritura de los consejos en inglés")
    print("7. Salir")
...
def ver_consejos():
    ...
    for idx, consejo in enumerate(consejos, 1):
        if COLOR_ENABLED:
            print(f"{Fore.GREEN}{idx}. Español: {consejo['español']}")
            print(f"   Inglés: {consejo['inglés']}{Style.RESET_ALL}\n")
        else:
...
def practicar_ortografia():
    ...
    if comparar_texto(consejo['español'], entrada_usuario):
        if COLOR_ENABLED:
            print(Fore.GREEN + "\n¡Correcto! Excelente trabajo." + Style.RESET_ALL)
        else:
            print("\n¡Correcto! Excelente trabajo.")
    else:
        if COLOR_ENABLED:
            print(Fore.RED + "\nHay algunos errores en tu escritura." + Style.RESET_ALL)
            print(f"Versión correcta: \"{consejo['español']}\"")
            diffs = difflib.ndiff([consejo['español']], [entrada_usuario])
            diferencias = ''.join(diffs)
            print("Diferencias:")
            print(Fore.YELLOW + diferencias + Style.RESET_ALL)
        else:
...
def practicar_traduccion_es_en():
    ...
    if comparar_texto(consejo['inglés'], entrada_usuario):
        if COLOR_ENABLED:
            print(Fore.GREEN + "\n¡Traducción correcta! Excelente trabajo." + Style.RESET_ALL)
        else:
            print("\n¡Traducción correcta! Excelente trabajo.")
    else:
        if COLOR_ENABLED:
            print(Fore.RED + "\nTraducción incorrecta." + Style.RESET_ALL)
...
def reescribir_consejos_1_100():
    print("\n=== Reescribir los Consejos del 1 al 100 ===\n")
    for idx in range(100):
        consejo = consejos[idx]
        numero = idx + 1
        print(f"Consejo #{numero} en Español:")
        if COLOR_ENABLED:
            print(Fore.GREEN + f"\"{consejo['español']}\"\n" + Style.RESET_ALL)
        else:
            print(f"\"{consejo['español']}\"\n")
        
        entrada_usuario = input("Escribe el consejo correctamente: ").strip()
        
        # Comparar la entrada del usuario con el consejo original
        if comparar_texto(consejo['español'], entrada_usuario):
            if COLOR_ENABLED:
                print(Fore.GREEN + "¡Correcto! Excelente trabajo.\n" + Style.RESET_ALL)
            else:
                print("¡Correcto! Excelente trabajo.\n")
        else:
            if COLOR_ENABLED:
                print(Fore.RED + "Hay algunos errores en tu escritura." + Style.RESET_ALL)
                print(f"Versión correcta: \"{consejo['español']}\"")
                diffs = difflib.ndiff([consejo['español']], [entrada_usuario])
                diferencias = ''.join(diffs)
                print("Diferencias:")
                ...
                print("Hay algunos errores en tu escritura.")
                print(f"Versión correcta: \"{consejo['español']}\"")
                diffs = difflib.ndiff([consejo['español']], [entrada_usuario])
                diferencias = ''.join(diffs)
                print("Diferencias:")
    ...
    input("Has completado la reescritura de los consejos del 1 al 100.\nPresiona Enter para volver al menú principal...")

def practicar_escritura_en_inglés():
    print("\n=== Practicar la Escritura de los Consejos en Inglés ===\n")
    for idx, consejo in enumerate(consejos, 1):
        print(f"Consejo #{idx} en Inglés:")
        if COLOR_ENABLED:
            print(Fore.BLUE + f"\"{consejo['inglés']}\"\n" + Style.RESET_ALL)
        else:
            print(f"\"{consejo['inglés']}\"\n")
        ...
        if comparar_texto(consejo['inglés'], entrada_usuario):
            ...
            if COLOR_ENABLED:
                print(Fore.RED + "Hay algunos errores en tu escritura." + Style.RESET_ALL)
                print(f"Versión correcta: \"{consejo['inglés']}\"")
                diffs = difflib.ndiff([consejo['inglés']], [entrada_usuario])
                diferencias = ''.join(diffs)
                print("Diferencias:")
                print(Fore.YELLOW + diferencias + Style.RESET_ALL)
                ...
                print(diferencias)
            print()  # Línea en blanco para mejor legibilidad
    input("Has completado la práctica de escritura en inglés de los 100 consejos.\nPresiona Enter para volver al menú principal...")
...
def main():
    while True:
        ...
        if opcion == '1':
            ...
            reescribir_consejos_1_100()
        elif opcion == '6':
            practicar_escritura_en_inglés()
        elif opcion == '7':
            print("\nGracias por usar el programa. ¡Hasta luego!")
            sys.exit()
        else:
            if COLOR_ENABLED:
                print(Fore.RED + "\nOpción no válida. Por favor, selecciona una opción del 1 al 7." + Style.RESET_ALL)
            else:
